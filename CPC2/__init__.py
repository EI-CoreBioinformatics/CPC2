...
import os
import re
import subprocess as sp  # commands
import time
...
def calculate_potential(fasta, strand, outfile):
    ...
    ftmp_result.write("\t".join(map(str, ["#ID", "transcript_length", "peptide_length", "Fickett_score", "pI",
    ...
    data_dir = os.path.join(os.path.dirname(pkg_resources.resource_filename("CPC2", "__init__.py")), "data") + os.path.sep
    lib_dir = os.path.join(os.path.dirname(pkg_resources.resource_filename("CPC2", "__init__.py")), "libs")
    ...
    sp.call('test -x ' + app_svm_scale + ' || echo \"[ERROR] No excutable svm-scale on CPC2 path!\" > /dev/stderr',
            shell=True)
    sp.call('test -x ' + app_svm_predict + ' || echo \"[ERROR] No excutable svm-predict on CPC2 path!\" > /dev/stderr',
            shell=True)
